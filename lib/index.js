...
/** Required host services. `connection` is provided by dsh-client-connection. */
...
function apply(ctx, config) {
	...
	// DSH >= 0.1.5 registers a channel's physical route through the caller
	// context's `webServer` service, so both services must be injected; the
	// `{ authority }` option of older releases is gone (trust and browser
	// authentication are applied by the connection service itself).
	ctx.inject(["connection", "webServer"], (connectionCtx) => {
		connectionCtx.connection.rpc.handle("/rpc-quota", handler);
	});
}
