...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function QuotaPill(props) {
			...
			// Initial load + quiet poll.
			useEffect(
				function () {
					var cancel = load(false);
			...
			// Manual refresh (also re-renders the pill after open).
				...
				open && pos
					? h(
							...
							h(
								...
								h(
									"button",
									{ type: "button", style: styles.refresh, onClick: refresh },
									fetching ? "刷新中…" : "刷新"
								)
							),
							h(
